...
class MainWindow():
    ...
    def __init__(self):
        ...
        #ui_list
        self.ui.Mjs_list.setSelectionMode(QtWidgets.QAbstractItemView.ExtendedSelection)
        self.ui.Mjs_list.itemSelectionChanged.connect(self.refreshSlotsFuntion)
        self.ui.Mjs_list_2.setSelectionMode(QtWidgets.QAbstractItemView.ExtendedSelection)
    ...
    def getMayaCurveEditor(self,pull=0,push=0,save=[]):
        ...
        getobjPyside = wrapInstance(int(getcontrol),QtWidgets.QWidget)
        return getobjPyside
        ...
        if nurbs:
            ...
            if itemsNum != 0:
                for ii in range(itemsNum):
                    ...
                    if vis == 0:
                        item.setForeground(QtGui.QColor(103,103,103))
                    else:
                        item.setForeground(QtGui.QColor(187,187,187))         
            if itemsNum2 != 0:
                for ii in range(itemsNum2):
                    item = self.ui.Mjs_list_2.item(ii)
                    vis = cmds.getAttr(item.text()+'.visibility')
